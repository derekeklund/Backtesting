...
def ATR(df, window):
    ...
    # Sum True Range over the window
    df['ATR'] = df['True_Range'].rolling(window=window).sum()

    # Divide by window size (average)
    df['ATR'] = df['ATR'] / window
...
stretch_sd = 1 # Moving Average Stretch standard deviation
...
for i in range(0, len(df)):
    ...
    if i == 0:
        ...
        trading_capital = 0.4 * df['port_value'].iloc[i-1] # 40% of portfolio value
...
# Save df to excel
df.to_excel(f"{ticker}_backtest.xlsx", index=False)
